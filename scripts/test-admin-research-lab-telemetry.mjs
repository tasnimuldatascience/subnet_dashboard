...
try {
  ...
  assert.match(routeSource, /LOOP_INDEX_BATCH_SIZE = 1_000/)
  assert.match(routeSource, /getCachedAdminLabLoopIndex\(supabase\)/)
  assert.match(routeSource, /\.range\(offset, offset \+ LOOP_INDEX_BATCH_SIZE - 1\)/)
  assert.match(routeSource, /buildAdminLabLoopPage\(loops, \{ page, query, status \}\)/)
  assert.match(routeSource, /researchLabStatusFilterOptionsWithCounts\(searchMatchedLoops\)/)
  assert.match(routeSource, /filterResearchLabActivityLoops\(searchMatchedLoops, \{ status \}\)/)
  assert.match(routeSource, /filteredLoops\.slice\(offset, offset \+ LOOP_LIMIT\)/)
  ...
  assert.match(adminComponentSource, /function useHoverPopover\(\)/)
  assert.match(adminComponentSource, /onMouseEnter=\{hoverPopover\.openPreview\}/)
  assert.match(adminComponentSource, /onMouseLeave=\{hoverPopover\.closePreview\}/)
  assert.match(adminComponentSource, /onFocus=\{hoverPopover\.openPreview\}/)
  assert.match(adminComponentSource, /onClick=\{hoverPopover\.togglePinned\}/)
  assert.match(adminComponentSource, /<PopoverAnchor asChild>/)
  assert.match(adminComponentSource, /onOpenAutoFocus=\{\(event\) => event\.preventDefault\(\)\}/)
  assert.doesNotMatch(adminComponentSource, /<PopoverContent[\s\S]{0,200}onMouseEnter=/)
