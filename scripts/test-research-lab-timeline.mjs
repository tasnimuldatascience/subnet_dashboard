...
try {
  ...
  assert.match(componentSource, /finalResultLabel = loop \? loop\.statusLabel \|\| readableTag\(loop\.outcomeLabel\) : undefined/)
  assert.match(componentSource, /<TimelineStageList[\s\S]*finalResultLabel=\{finalResultLabel\}/)
  assert.match(componentSource, /<OutcomeBadge label=\{resultLabel\} band=\{resultBand \?\? 'pending'\} \/>/)
