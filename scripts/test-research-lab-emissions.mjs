...
try {
  ...
  assert.match(componentSource, /Live metagraph emission for every current Lab recipient, including queued champions\./)
  ...
  assert.match(componentSource, /Object\.keys\(spend\?\.currentAllocation\?\.byHotkey \?\? \{\}\)/)
  assert.match(componentSource, /row\.computeSpendUsd > 0 \|\| row\.hasCurrentReward \|\| row\.isCurrentLabRecipient/)
